...
import { Center, Badge, View, Text, Heading, Input, Button, Box, Flex } from 'native-base'
...
function AddCate() {
  ...
  const handleOnPress = async () => {
    try {
      const token = await AsyncStorage.getItem('token');
      if (token !== null) {
        const response = await API.post('/categori', cate);
        getCate()
        console.log(response);
      }
    } catch (error) {
  ...
  return (
    <View px="10" py="20" height={hp("100%")}>
      ...
      <Input
        ...
        value={cate.name}
      />
      <Button style={{ backgroundColor: "#FF5555" }} onPress={handleOnPress}>
        Add New Category
      </Button>
